...
Para cada bloque, generá una `frase` corta (consejo general para ese momento) y un `nivel` (favorable/neutro/precaucion). NO generes `accionables` — ese campo se completa desde otra fuente. Dejá `accionables` como array vacío `[]`.
...
Nunca generes frases genéricas tipo horóscopo de revista. Todo debe ser específico.
...
Las frases de cada momento deben ser concretas y contextuales, no genéricas.
